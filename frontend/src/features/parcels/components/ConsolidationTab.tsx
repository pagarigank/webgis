...
export function ConsolidationTab({ initialParcelId }: ConsolidationTabProps) {
    ...
    const [selected, setSelected] = useState<string[]>(initialParcelId ? [initialParcelId] : []);
    ...
    // Live versions for the selected parents (If-Match material for commit).
    const versionMap = useMemo(() => {
        const map: Record<string, number> = {};
        for (const p of candidates?.data ?? []) {
            if (selected.includes(p.id)) map[p.id] = p.version;
        }
        return map;
    }, [candidates, selected]);
    ...
    const selectedRows = selected.map((id) => candidatesById.get(id)).filter((r): r is NonNullable<typeof r> => r != null);
        ...
        <div data-testid="consolidation-tab">
            <div className="row g-4">
                <div className="col-lg-5">
                    <label className="form-label small fw-semibold" htmlFor="consolidation-search">
                        Find parcels
                    </label>
                    <input
                    ...
                    <div className="mb-2">
                        <label className="form-label small" htmlFor="consolidation-lot">
                            New parcel lot number
                        </label>
                        <input id="consolidation-lot" data-testid="consolidation-lot" className="form-control form-control-sm" value={form.lotNumber} onChange={(e) => setForm((f) => ({ ...f, lotNumber: e.target.value }))} />
                    </div>
                    <div className="mb-2">
                        <label className="form-label small" htmlFor="consolidation-remarks">
                            New parcel remarks
                        </label>
                        <input id="consolidation-remarks" data-testid="consolidation-remarks" className="form-control form-control-sm" value={form.remarks} onChange={(e) => setForm((f) => ({ ...f, remarks: e.target.value }))} />
                    </div>
                    <div className="form-check mb-2">
                        ...
                        />
                        <label className="form-check-label small" htmlFor="consolidation-multipart">
                            Allow multipart union (non-contiguous consolidation)
                        </label>
                    </div>
                    <div className="mb-2">
                        <label className="form-label small" htmlFor="consolidation-reason">
                            Reason (required for commit)
                        </label>
                        <input id="consolidation-reason" data-testid="consolidation-reason" className="form-control form-control-sm" value={form.reason} onChange={(e) => setForm((f) => ({ ...f, reason: e.target.value }))} />
                    </div>
                    ...
                    <button
                        ...
                        disabled={!canCommitConsolidation(preview) || form.reason.trim() === '' || commitMutation.isPending}
